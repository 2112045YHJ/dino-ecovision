...
public class MissionAssignmentService {
	...
	// 온보딩 직후 호출: 독립 트랜잭션(REQUIRES_NEW)으로 돌려, 실패해도 호출자(부화)에 영향 없게 한다.
	// 멱등: 오늘 배정이 이미 있으면 아무것도 하지 않는다(중복 INSERT 방지).
	...
	public void assignForUser(Long userId, LocalDate today) {
		...
		if (day.isEmpty() || evening.isEmpty() || anytime.isEmpty()) {
			log.warn("[ASSIGN] missing slot missions (DAY:{}, EVENING:{}, ANYTIME:{}). skip user {}.", day.size(),
					evening.size(), anytime.size(), userId);
			return;
		}
	...
	// ===== 스케줄러와 공유하는 선정 로직 (스케줄러에서 가져옴) =====

	public List<Mission> bySlot(List<Mission> all, String slot) {
		return all.stream().filter(m -> slot.equalsIgnoreCase(m.getSlotType())).collect(Collectors.toList());
	}

	public List<Mission> selectNonOverlappingMissions(List<Mission> dayList, List<Mission> eveningList,
			List<Mission> anytimeList) {
