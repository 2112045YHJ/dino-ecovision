...
## 🔧 [신규] 대시보드 내 차트 저장 버튼 구현 및 에디터 불러오기 연동 (2026-06-22)

대시보드 상에서 분석된 차트 상태를 내 계정의 보관함에 영구 보관할 수 있는 명시적인 저장(💾) 기능을 추가하고, 저장 완료된 스냅샷들이 게시글 작성(에디터) 차트 공유 불러오기 모달에 자동으로 동기화되어 본문에 간편히 선택/삽입될 수 있도록 흐름을 고도화했습니다.

### 1. 수정 및 조치 내역

1. **[Dashboard.tsx](file:///c:/final/dino-ecovision/frontend/src/pages/Dashboard.tsx) [MODIFY]**
   - **💾 차트 저장 기능 구현**: `handleSaveChartSnapshot` 비동기 저장 함수를 신설하여 단일/비교 차트 상태 데이터를 스냅샷 API로 보내고 데이터베이스에 유저 권한(`AuthenticationPrincipal`)으로 저장시켰습니다. 저장 성공 시 내 보관함에 저장 완료되었다는 안내 토스트 메시지를 띄웁니다.
   - **UI 및 라벨 정돈**: 상단 액션 버튼 그룹에 `"💾 차트 저장"` 단추를 신설 배치하고, 기존의 `"🔗 공유하기"` 버튼 텍스트는 클립보드 주소 복사 용도를 명확히 하기 위해 `"🔗 링크 복사"` 로 직관적으로 수정했습니다.

2. **[CommunityWritePage.tsx](file:///c:/final/dino-ecovision/frontend/src/pages/CommunityWritePage.tsx) (연동 정합성 검증)**
   - 대시보드에서 보관함에 저장한 스냅샷 정보들이 글쓰기 에디터 상단 툴바 `+: (플러스)` ➡️ `저장한 차트 추가` 모달 창 목록에 실시간으로 완벽하게 연계되어 표시되고, 클릭 시 본문 커서 부근에 `/embed/{uuid}` 로 정확히 텍스트 주입되어 임베딩 차트 위젯이 렌더링됨을 검증했습니다.

### 2. 검증 결과
* **프론트엔드 빌드 무결성**: `npm run build` 결과 에러 없이 빌드 검증을 100% 통과했습니다.
